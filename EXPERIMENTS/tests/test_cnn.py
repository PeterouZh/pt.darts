...
class TestingTrainSearch(unittest.TestCase):
  ...
  def test_cnn_cifar10_augment(self):
    """
    Usage:
        source activate Peterou_torch_py36
        ...
        export PYTHONPATH=../../submodule:../..
        python -c "import test_cnn; \
        test_cnn.TestingTrainSearch().test_cnn_cifar10_augment()"
    ...
    args, myargs = utils.config.setup_args_and_myargs(args=args, myargs=myargs)
    import augment
    augment.run(args, myargs)
    input('End %s' % outdir)
    return

  def test_cnn_cifar10_augment_pgd(self):
    """
    Usage:
        source activate Peterou_torch_py36
        export CUDA_VISIBLE_DEVICES=0
        ...
        export PYTHONPATH=../../submodule:../..
        python -c "import test_cnn; \
        test_cnn.TestingTrainSearch().test_cnn_cifar10_augment_pgd()"
    ...
    def build_args():
      argv_str = f"""
            --config configs/cnn_cifar10.yaml
            --command cnn_cifar10_augment_pgd
            --resume False --resume_path None
            --resume_root None
            """
      parser = utils.args_parser.build_parser()
